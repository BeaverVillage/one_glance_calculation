...
export function initRentVsJeonseCalculator(root = document) {
  ...
  const els = {
    ...
    placeModal: root.querySelector('#rent-place-modal'),
    placeModalSummary: root.querySelector('#rent-place-modal-summary'),
    placeModalCloseButtons: root.querySelectorAll('[data-rent-place-close]'),
    marketShortcut: root.querySelector('#rent-market-shortcut'),
    marketPanel: root.querySelector('#rent-market-panel')
  };
  ...
  root.addEventListener('keydown', (event) => {
    if (event.key === 'Escape') closePlaceModal(els);
  });

  if (els.marketShortcut && els.marketPanel) {
    els.marketShortcut.addEventListener('click', () => {
      els.marketPanel.scrollIntoView({ behavior: 'smooth', block: 'start' });
      window.setTimeout(() => els.placeQuery?.focus({ preventScroll: true }), 450);
    });
  }
}
...
function renderPlaceCandidates(root, els, items) {
  ...
  if (els.placeModalSummary) {
    els.placeModalSummary.textContent = query
      ? `검색어: ${query} · 결과 ${items.length}개`
  ...
  if (els.apiStatus) els.apiStatus.textContent = '팝업에서 아파트나 지역을 선택해 주세요.';
  openPlaceModal(els);
}
