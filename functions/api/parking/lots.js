...
export async function onRequestGet({ request, env }) {
  ...
  const external = await resolveParkingLotDataset({ env, destination: center, radius, query: url.searchParams.get('q') || '' });
  const sourceLots = external.lots.length ? external.lots : parkingLots;
  const lots = sourceLots.filter((lot) => lot.lat && lot.lng)
    .map((lot) => ({ ...lot, distanceKm: Math.round(distanceKm(center, lot) * 10) / 10 }))
    .filter((lot) => lot.distanceKm * 1000 <= radius || radius >= 3000)
    .sort((a, b) => a.distanceKm - b.distanceKm)
    .slice(0, 50);
  return json({ lots, radius, dataMode: external.lots.length ? 'public-adapter' : 'sample-fallback', dataSources: external.meta.sources || [] });
}
